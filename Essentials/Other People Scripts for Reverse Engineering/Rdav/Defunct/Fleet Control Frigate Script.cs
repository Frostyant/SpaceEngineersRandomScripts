...
----------------------------  
    ...
    You are currently looking at the 'Frigate' module used on frigates.  
    It requires:  
    ~Correctly oriented gyros (up pointing forward) 
    ~An Rmote control unit facing forward 
    ~A connector with its behind facing the bottom of the Rc unit 
    ~Any sensors set to detect enemy targets  
 
    Make sure your ship can stop in UNDER 2000m!! 
...
//User Inputs For Custom Ships:                 
//--------------------------------------------                    
double BRAKING_DISTANCE = 2500;             //Distance required for a ship to reach full stop from 100m/s                    
float SHIPRADIUS = 40;                                    //Largest diameter of ship                    
double MAX_VELOCITY = 60;                           //Formation cruising velocity, (less than 90 m/s)         
...
Latest Changenotes                 
-------------------------------------------------------------------------------------       
~Adjusted collect functions for precise thrust to collect only on same grid 
~Adjusted collect functions to collect large thrusters too 
~Adjusted PID values to better orient the frigate (all gyro to 1)    
~Updated from FighterAI     
*/           
...
/*==#=====#====#=====#=====#=====#=====#=====#====#=====#=====#=====#=====#=====#====#===================                                 
  High Level Function: FrigateControl                                
  ------------------------------------------------                                
  function will: Give Frigates Rudimentary AI and combat capabilities                               
...
//==================================                        
          ...
          {if(CONNECTORS[j].CubeGrid == Me.CubeGrid)                               
           ...
           if(COMMAND == "Attack" )                         
                    {                    
                        ...
                        COORDSHASRUN = false;                    
                         ...
                         //If Distance is Smaller Than 820m, Point To Target And Attack      
                         //------------------------------------------------------------------------------------                   
                         if(D_T_TARGET < 820)                        
                             {      
                              ...
                              //Applying Targetposition      
                              //---------------------------------            
                              IMyGyro GYRO = GYROS[0] as IMyGyro;         
                              GyroTurn(AIM_POINT, GYRO, 1 );                      
                              for(int j = 0; j < GYROS.Count;j++)                                                                  
                                  {                                     
                                  var GYROA = GYROS[j] as IMyGyro;                                   
...
//================================================================              
 if(OPERATION == "Dock")           
     ...
     //-------------------------------------------------------------                               
      float ROLLANGLE = (float)(1*(Vector3D.Dot(Vector3D.Normalize(CONPOS-READYPOS), Vector3D.Normalize(ExtractDirection("RIGHT", RC_UNIT)))));             
      float YAW_VEL = (float)(0.5*((ROLLANGLE - PREV_MISSILEROLL)/(double)(ElapsedTime.TotalSeconds)));        
...
{                                      
    ...
    //================================================================                                
       ...
       //================================================================                                
            ...
            if(DISTANCE_TO_TARGET>100+SHIPRADIUS)                            
             {                       
                GyroTurn(TARGETPOSITION, GYRO, 1 );                     
                for(int j = 0; j < GYROS.Count;j++)                                                                 
                    {                                     
                    var GYROA = GYROS[j] as IMyGyro;         
                    GYROA.SetValue<float>("Yaw", (0));                                   
